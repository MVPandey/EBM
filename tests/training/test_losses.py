...
def _make_jepa_output(b: int = 4, d: int = 32) -> JEPAOutput:
    ...
    return JEPAOutput(energy=energy, z_pred=z_pred, z_target=z_target, decode_logits=logits)
...
def test_compute_loss_returns_loss_output():
    ...
    result = compute_loss(out, _make_solution(), TrainingConfig())
...
def test_compute_loss_components_positive():
    ...
    result = compute_loss(out, _make_solution(b=8), TrainingConfig())
