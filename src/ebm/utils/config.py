...
class TrainingConfig(BaseModel):
    ...
    decode_loss_weight: float = Field(default=0.1, description='Weight for auxiliary decoder cross-entropy loss.')
    vicreg_sim_weight: float = Field(default=25.0, description='VICReg similarity (invariance) loss weight.')
    vicreg_var_weight: float = Field(default=25.0, description='VICReg variance loss weight.')
    vicreg_cov_weight: float = Field(default=1.0, description='VICReg covariance loss weight.')
